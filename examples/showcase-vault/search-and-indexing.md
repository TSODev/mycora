...
- **Full-text search** — `/` opens a live-as-you-type overlay over
  ...
  each match, matched terms highlighted.
...
- **Manual/watched reindex** — `mycora reindex` rebuilds on demand;
  `mycora reindex --watch` keeps it in sync as files change on disk.
